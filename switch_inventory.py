import yaml
from datetime import datetime

# Switch data — update IPs and MACs as needed
switches = {
    "local_switch": {
        "ansible_host": "10.10.1.24",
        "general_settings": {
            "name": "Local_Switch",
            "ram": "512Mb",
            "vcpus": 1,
            "qemu_binary": "/usr/bin/qemu-system-x86_64 (v4.2.1)",
            "boot_priority": "CD/DVD-ROM or HDD",
            "on_close": "Power off the VM",
            "console_type": "telnet"
        },
        "network_settings": {
            "adapters": 13,
            "base_mac": "0c:c0:5e:66:00:00",
            "nic_type": "Realtek 8139 Ethernet (rtl8139)",
            "replicate_state": True
        }
    },
    "user_network": {
        "ansible_host": "10.10.1.22",
        "general_settings": {
            "name": "User_Network",
            "ram": "512Mb",
            "vcpus": 1,
            "qemu_binary": "/usr/bin/qemu-system-x86_64 (v4.2.1)",
            "boot_priority": "CD/DVD-ROM or HDD",
            "on_close": "Power off the VM",
            "console_type": "telnet"
        },
        "network_settings": {
            "adapters": 13,
            "base_mac": "0c:e0:f2:0b:00:00",
            "nic_type": "Realtek 8139 Ethernet (rtl8139)",
            "replicate_state": True
        }
    },
    "acct_network": {
        "ansible_host": "10.10.1.32",
        "general_settings": {
            "name": "ACCT_Network",
            "ram": "512Mb",
            "vcpus": 1,
            "qemu_binary": "/usr/bin/qemu-system-x86_64 (v4.2.1)",
            "boot_priority": "CD/DVD-ROM or HDD",
            "on_close": "Power off the VM",
            "console_type": "telnet"
        },
        "network_settings": {
            "adapters": 13,
            "base_mac": "0c:40:34:07:00:00",
            "nic_type": "Realtek 8139 Ethernet (rtl8139)",
            "replicate_state": True
        }
    },
    "mgmt_network": {
        "ansible_host": "10.10.1.31",
        "general_settings": {
            "name": "MGMT_Network",
            "ram": "512Mb",
            "vcpus": 1,
            "qemu_binary": "/usr/bin/qemu-system-x86_64 (v4.2.1)",
            "boot_priority": "CD/DVD-ROM or HDD",
            "on_close": "Power off the VM",
            "console_type": "telnet"
        },
        "network_settings": {
            "adapters": 13,
            "base_mac": "0c:cc:78:5d:00:00",
            "nic_type": "Realtek 8139 Ethernet (rtl8139)",
            "replicate_state": True
        }
    },
    "it_network": {
        "ansible_host": "10.10.1.30",
        "general_settings": {
            "name": "IT_Network",
            "ram": "512Mb",
            "vcpus": 1,
            "qemu_binary": "/usr/bin/qemu-system-x86_64 (v4.2.1)",
            "boot_priority": "CD/DVD-ROM or HDD",
            "on_close": "Power off the VM",
            "console_type": "telnet"
        },
        "network_settings": {
            "adapters": 13,
            "base_mac": "0c:1c:b2:85:00:00",
            "nic_type": "Realtek 8139 Ethernet (rtl8139)",
            "replicate_state": True
        }
    }
}

# Wrap into standard Ansible structure
inventory = {
    "all": {
        "hosts": switches,
        "vars": {
            "ansible_user": "ssh_user",
            "ansible_ssh_pass": "P@ssw0rd",
            "ansible_connection": "network_cli",
            "ansible_network_os": "exos"
        }
    }
}

# Output filename
timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
filename = f"switch_inventory_{timestamp}.yml"

# Save to file
with open(filename, "w") as f:
    yaml.dump(inventory, f, sort_keys=False)

print(f"Inventory saved to: {filename}")